...
def _configured_tokens() -> dict[str, str]:
    ...
    tokens = {}
    broker_token = os.environ.get("BROKER_AGENT_TOKEN", "").strip()
    admin_token = os.environ.get("ADMIN_AGENT_TOKEN", "").strip()
    service_token = os.environ.get("AGENT_SERVICE_TOKEN", "").strip()
    if broker_token:
        tokens["broker"] = broker_token
    if admin_token:
        tokens["admin"] = admin_token
    if service_token:
        tokens["orchestrator"] = service_token
    return tokens
